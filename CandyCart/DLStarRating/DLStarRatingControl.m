...
- (id)initWithFrame:(CGRect)frame andStars:(NSUInteger)_numberOfStars isFractional:(BOOL)isFract starGapCustomAdjust:(float)gapAdjust{
	self = [super initWithFrame:frame];
	if (self) {
        starGapCustomAdjust = gapAdjust;
...
- (void)layoutSubviews {
	for (int i=0; i < numberOfStars; i++) {
		[(DLStarView*)[self subViewWithTag:i] centerIn:self.frame with:numberOfStars starGapAdjust:starGapCustomAdjust];
	}
}
