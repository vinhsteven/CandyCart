...
- (void)initInfiniteScrollView
{
    ...
    if (_imageAry.count > 0)
    {
        ...
        for (int i = 0; i < (_imageAry.count*5); i++)
        {
            ...
            UILabel *temp = [[UILabel alloc] initWithFrame:CGRectMake(i * _itemSize.width, self.frame.size.height - _itemSize.height, _itemSize.width, _itemSize.height)];
            temp.text = [_imageAry objectAtIndex:i%_imageAry.count];
            temp.textAlignment = NSTextAlignmentCenter;
            temp.backgroundColor = [UIColor clearColor];
            
            temp.layer.cornerRadius= _itemSize.width/2;
