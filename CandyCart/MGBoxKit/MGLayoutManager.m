...
+ (void)stackByZIndexIn:(UIView *)container {
  NSArray *sorted =
      [container.subviews sortedArrayUsingComparator:^NSComparisonResult(id view1,
          id view2) {
        int z1 = [view1 respondsToSelector:@selector(zIndex)] ? [(MGScrollView*)view1 zIndex] : 0;
        int z2 = [view2 respondsToSelector:@selector(zIndex)] ? [(MGScrollView*)view2 zIndex] : 0;
        if (z1 > z2) {
